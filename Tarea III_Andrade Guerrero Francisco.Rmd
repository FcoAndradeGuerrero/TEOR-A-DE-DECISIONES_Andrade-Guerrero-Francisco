...
ggplot()+
  geom_line(aes(x,y), color="aquamarine4", , linewidth=1.5)+
  geom_function(fun = fv_den, color="blue4", linewidth=0.5)+
  xlim(-1.5, 1.5)+
  theme_bw()
...
Centro_nsombreada_v <- fv_den(Centro_nsombreada_h)/2
  ...
  ggplot()+
  geom_function(fun = fv_den, color="aquamarine", linewidth=0.5, xlim=c(-B-0.1, B+0.1))+
  stat_function(fun = fv_den, geom= "area", fill="brown1", alpha=0.5, xlim=c(-B, Vji))+ 
  stat_function(fun = fv_den, geom = "area", fill = "burlywood", alpha = 0.5, xlim = c(Vji, B))+
